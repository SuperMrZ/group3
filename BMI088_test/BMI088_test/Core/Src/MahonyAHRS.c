...
#define twoKpDef	(2.0f * 5.0f)	// 比例增益的两倍
#define twoKiDef	(2.0f * 1.0f)	// 积分增益的两倍
...
//读取bmi088数据函数
void Read_Sensors(acc_raw_data_t *accel, gyro_raw_data_t *gyro,ist8310_raw_data_t *ist8310) {

     ReadAccData(accel); 
	 ReadGyroData(gyro);
	 ReadIST8310Data(ist8310);
	
    // 更新四元数
//     MahonyAHRSupdateIMU(gyro->roll,gyro->pitch,gyro->yaw,accel->x,accel->y,accel->z);
	 MahonyAHRSupdate(gyro->roll,gyro->pitch,gyro->yaw,accel->x,accel->y,accel->z, ist8310->x, ist8310->y, ist8310->z);
}
